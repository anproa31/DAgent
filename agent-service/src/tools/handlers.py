...
from infrastructure.app.web_datasource_client import (
    discover_web_data,
    fetch_web_url,
    propose_web_data,
    register_web_data,
    summarize_discover_response,
)
...
def _discover_tool_result(response: Dict[str, Any], *, proposal_only: bool = False) -> ToolResult:
    datasources: List[Dict[str, Any]] = response.get("datasources") or []
    errors: List[str] = response.get("errors") or []
    selected_urls: List[str] = response.get("selected_urls") or []
    candidates = response.get("candidates") or []

    if proposal_only:
        if not selected_urls:
            err = "; ".join(errors) if errors else "No dataset URLs proposed"
            return ToolResult(
                success=False,
                chunks=[text_chunk(f"Web search failed: {err}")],
                data={"candidates": candidates, "selected_urls": selected_urls},
                error=err,
            )
        summary = summarize_discover_response(response)
        chunks = [text_chunk(summary)]
        for cand in candidates[:5]:
            chunks.append(
                text_chunk(
                    f"Candidate ({cand.get('score', 0)}): {cand.get('title', '')} — {cand.get('url', '')}"
                )
            )
        return ToolResult(
            success=True,
    ...
    if not datasources:
        err = "; ".join(errors) if errors else "No datasets registered"
        chunks = [text_chunk(f"Web discovery failed: {err}")]
        for cand in candidates[:5]:
            chunks.append(
                text_chunk(f"Candidate: {cand.get('title', '')} — {cand.get('url', '')}")
            )
        return ToolResult(
            success=False,
            chunks=chunks,
    ...
    for ds in datasources:
        view_names.extend(ds.get("view_names") or [])
        if ds.get("id"):
    ...
    for url in selected_urls:
        chunks.append(text_chunk(f"Source: {url}"))
...
async def handle_propose_web_data(
    session_id: str,
    query: str,
    ...
    if not query and not url:
        return ToolResult(
            success=False,
            chunks=[text_chunk("Provide a search query or direct data URL")],
            error="Missing query and url",
        )
    response = await propose_web_data(
        query or url or "",
        url=url,
        model=model,
        base_url=base_url,
    ...
    return _discover_tool_result(response, proposal_only=True)


async def handle_register_web_data(
    session_id: str,
    urls: List[str],
    ...
    if not urls:
        return ToolResult(
            success=False,
            chunks=[text_chunk("At least one approved URL is required")],
            error="Missing urls",
        )
    response = await register_web_data(urls, name=name, query=query)
    return _discover_tool_result(response, proposal_only=False)


async def handle_discover_web_data(
    session_id: str,
    query: str,
    url: Optional[str] = None,
    name: Optional[str] = None,
    ...
    response = await discover_web_data(
        query or url or "",
        url=url,
        name=name,
        model=model,
...
async def handle_fetch_web_data(
    session_id: str,
    url: str,
    name: Optional[str] = None,
) -> ToolResult:
    if not url:
        return ToolResult(
            success=False,
            chunks=[text_chunk("URL is required")],
            error="Missing url",
        )
    response = await fetch_web_url(url, name=name)
    return _discover_tool_result(response, proposal_only=False)
